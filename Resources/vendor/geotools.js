...
(function(API) {
	 ...
	 */
	API.decode_bbox = function(geohash) {
		var lat_interval = [-90.0, 90.0];
		var lon_interval = [-180.0, 180.0];
	...
	API.encode = function(latitude, longitude, length) {
		if (!length)
	 ...
	 *         box.
	 */
	API.north = function(geoHash) {
		var bbox = API.decode_bbox(geoHash);
		var latDiff = bbox[1] - bbox[0];
		var lat = bbox[0] - latDiff / 2;
		var lon = (bbox[2] + bbox[3]) / 2;
		return API.encode(lat, lon, geoHash.length);
	};
	 ...
	 *         geo hash
	 */
	API.geohashContains = function(geoHash, latitude, longitude) {
		return API.bboxContains(API.decode_bbox(geoHash), latitude, longitude);
	};
	 ...
	 */
	API.subHashes = function(geoHash) {
		var list = [];
	 ...
	 */
	API.subHashesN = function(geoHash) {
		var list = [];
		var n = 0;
	 ...
	 */
	API.subHashesS = function(geoHash) {
		var list = [];
		var n = 0;
		for (var i in BASE32_CHARS) {
			var c = BASE32_CHARS[i];
	 ...
	 */
	API.subHashesNW = function(geoHash) {
		var list = [];
		var n = 0;
		for (var i in BASE32_CHARS) {
			var c = BASE32_CHARS[i];
	 ...
	 */
	API.subHashesNE = function(geoHash) {
		var list = [];
		var n = 0;
		for (var i in BASE32_CHARS) {
			var c = BASE32_CHARS[i];
	 ...
	 */
	API.subHashesSW = function(geoHash) {
		var list = [];
		var n = 0;
		for (var i in BASE32_CHARS) {
			var c = BASE32_CHARS[i];
	 ...
	 */
	API.getGeoHashesForPolygon = function(polygonPoints, maxLength) {
		...
		var bbox = API.bboxForPolygon(polygonPoints);
		// first lets figure out an appropriate geohash length
		var diagonal = API.distance(bbox[0], bbox[2], bbox[1], bbox[3]);
		var hashLength = API.getSuitableHashLength(diagonal, bbox[0], bbox[2]);

		var partiallyContained = [];
		// now lets generate all geohashes for the containing bounding box
		// lets start at the top left:

		var rowHash = API.encode(bbox[0], bbox[2], hashLength);
		var rowBox = API.decode_bbox(rowHash);
		while (rowBox[0] < bbox[1]) {
			var columnHash = rowHash;
			var columnBox = rowBox;

			while (API.isWest(columnBox[2], bbox[3])) {
				partiallyContained.push(columnHash);
				columnHash = API.east(columnHash);
				columnBox = API.decode_bbox(columnHash);
			}

			// move to the next row
			rowHash = API.south(rowHash);
			rowBox = API.decode_bbox(rowHash);
		}
		var fullyContained = [];
	...
	function splitAndFilter(polygonPoints, fullyContained, partiallyContained) {
		...
		for (var i = 0; i < partiallyContained.length; i++) {
			...
			var subHashes = API.subHashes(hash);
			for (var j = 0; j < subHashes.length; j++) {
				var h = subHashes[j];
				var hashBbox = API.decode_bbox(h);
				var nw = API.polygonContains(polygonPoints, hashBbox[0], hashBbox[2]);
				var ne = API.polygonContains(polygonPoints, hashBbox[0], hashBbox[3]);
				var sw = API.polygonContains(polygonPoints, hashBbox[1], hashBbox[2]);
				var se = API.polygonContains(polygonPoints, hashBbox[1], hashBbox[3]);
				if (nw && ne && sw && se) {
					...
					for (var k = 1; k < polygonPoints.length; k++) {
						...
						if (API.linesCross(hashBbox[0], hashBbox[2], hashBbox[0], hashBbox[3], last[0], last[1], current[0], current[1])) {
							stillPartial.push(h);
							break;
						} else if (API.linesCross(hashBbox[0], hashBbox[3], hashBbox[1], hashBbox[3], last[0], last[1], current[0], current[1])) {
							stillPartial.push(h);
							break;
						} else if (API.linesCross(hashBbox[1], hashBbox[3], hashBbox[1], hashBbox[2], last[0], last[1], current[0], current[1])) {
							stillPartial.push(h);
							break;
						} else if (API.linesCross(hashBbox[1], hashBbox[2], hashBbox[0], hashBbox[2], last[0], last[1], current[0], current[1])) {
							stillPartial.push(h);
							break;
						}
					}
	 ...
	 */
	API.getSuitableHashLength = function(granularityInMeters, latitude, longitude) {
		if (granularityInMeters < 5) {
			return 10;
		}
		var hash = API.encode(latitude, longitude);
		var width = 0;
		...
		while (width < granularityInMeters && hash.length >= 2) {
			length = hash.length;
			var bbox = API.decode_bbox(hash);
			width = API.distance(bbox[0], bbox[2], bbox[0], bbox[3]);
	 ...
	 *         length.
	 */
	API.geoHashesForLine = function(width, lat1, lon1, lat2, lon2) {
		if (lat1 == lat2 && lon1 == lon2) {
			throw new error("identical begin and end coordinate: line must have two different points");
		}

		var hashLength = API.getSuitableHashLength(width, lat1, lon1);
		...
		if (result1[0] == result2[0]) {// same geohash for begin and end
			...
			return API.getGeoHashesForPolygon([[bbox1[0], bbox1[2]], [bbox1[1], bbox1[2]], [bbox2[1], bbox2[3]], [bbox2[0], bbox2[3]]], hashLength);
		} else {
			return API.getGeoHashesForPolygon([[bbox1[0], bbox1[2]], [bbox1[0], bbox1[3]], [bbox2[1], bbox2[2]], [bbox2[1], bbox2[3]]], hashLength);
		}
	};
	...
	API.geoHashesForCircle = function(length, latitude, longitude, radius) {
		...
		var segments;
		var suitableHashLength = API.getSuitableHashLength(radius, latitude, longitude);
		...
		var circle2polygon = API.circle2polygon(segments, latitude, longitude, radius);
		return API.getGeoHashesForPolygon(circle2polygon, length);
	};
	/**
	 * @param geoHash
	 * @return the 8 south-east sub hashes of the geo hash
	 */
	API.subHashesSE = function(geoHash) {
		var list = [];
		var n = 0;
		for (var i in BASE32_CHARS) {
			var c = BASE32_CHARS[i];
	...
	API.isNorth = function(l1, l2) {
		return l1 > l2;
	};

	API.isSouth = function(l1, l2) {
		return l1 < l2;
	};

	API.bboxForPolygon = function(polygonPoints) {
		var minLat = 91;
		var minLon = 181;
		var maxLat = -91;
		var maxLon = -181;
	 ...
	 */
	API.bboxContains = function(bbox, latitude, longitude) {
	 ...
	 */
	API.polygonContains = function(polygonPoints, latitude, longitude) {
		...
		var bbox = API.bboxForPolygon(polygonPoints);
		if (!API.bboxContains(bbox, latitude, longitude)) {
		...
		for (var i = 0; i < polygonPoints.length; lastLatitude = currentLatitude,
		lastLongitude =
		currentLongitude, i++) {
	 ...
	 */
	API.roundToDecimals = function(d, decimals) {
		if (decimals > 17) {
	...
	API.linesCross = function(x1, y1, x2, y2, u1, v1, u2, v2) {
	 ...
	 */
	API.translateLongitude = function(latitude, longitude, meters) {
	 ...
	 * @return the translated coordinate.
	 */
	API.translateLatitude = function(latitude, longitude, meters) {
	 ...
	 * @return the translated coordinate.
	 */
	API.translate = function(latitude, longitude, lateralMeters, longitudalMeters) {
		var longitudal = API.translateLongitude(latitude, longitude, longitudalMeters);
		return API.translateLatitude(latitude, longitudal[1], lateralMeters);
	};
	 ...
	 */
	API.getPolygonCenter = function(polygonPoints) {
		var cumLat = 0;
		var cumLon = 0;
		polygonPoints.forEach(function(p) {
			cumLat += parseFloat(p.lat);
			cumLon += parseFloat(p.lon);
		});
		return {
			lat : cumLat / polygonPoints.length,
			lon : cumLon / polygonPoints.length
		};
	};

	API.bbox2polygon = function(bbox) {
	 ...
	 *         polygon.
	 */
	API.circle2polygon = function(segments, latitude, longitude, radius) {
		if (segments < 5) {
	...
	API.getPolygonForPoints = function(points) {
		if (points.length < 3) {
	 ...
	 */
	API.toDecimalDegree = function(direction, degrees, minutes, seconds) {
		var factor = 1;
