...
module.exports = function(httpbody) {
	if (!httpbody || !httpbody.responseText) {
		return {
			total : 0,
			nodes : [],
		};
	...
	var allnodes = [];
	/// XML:
	// beginnt mit optionalem Leerzeichen und '<?xml'
	console.log(httpbody.responseText.substr(0, 100));

	if (httpbody.responseText.match(/^\s*<html/mig)) {
		// HTML  Error page??
	} else if (httpbody.responseText.match(/^\s*<\?xml/mg)) {
		console.log('PARSERINFO: XML found');
		var xml = httpbody.responseXML;
		if (xml.documentElement) {
			allnodes = parseXML(xml);
		}

	} else
		allnodes = parseJSON(httpbody.responseText);
