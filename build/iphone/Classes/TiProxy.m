...
//Rare exceptions to indicate a bug in the _freifunker code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_freifunker = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_freifunker]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.freifunker.module"
